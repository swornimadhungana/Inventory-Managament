...
<html lang="en">
  ...
  <body>
    ...
    ?></div></div>
      ...
      <li class="add">Edit Suppliers</li>
      </ul>
    </nav>
    ...
    <div class="wrapper">
      <div class="container">
      <form method="post" action="../Function/editsupplier.php">
        <?php 
            if($_GET){
                $id = $_GET["id"];
            }
            ...
            $res = mysqli_query($conn, "SELECT * FROM `supplier` WHERE `Supplier_ID` = '$id'");
            if($res){
                while($row = mysqli_fetch_assoc($res)){
?>
    <label for="name" >Supplier Name:</label><input type="text" placeholder="Supplier Name" id="name" required name="suppliername" 
    <?php echo "value='".$row['Supplier_Name']."'";?>>
    
     <label for="email" >Supplier Email:</label><input type="text" placeholder="Supplier Email" id="email" required name="supplieremail"
<?php echo "value='".$row['Supplier_Email']."'";?>>
     <label for="address" >Supplier Address:</label><input type="text" placeholder="Supplier Address" id="address" required name="supplieraddress"
<?php echo "value='".$row['Supplier_Address']."'";?>>
     <div class="row">
  <label for="status" >Status:</label><select name="supplierstatus" id="status"> <!--id lekheko css ko lagi anii name lekheko php ko lagi -->
    <option value="active">Active</option>
    <option value="inactive">Inactive</option>
  </select>
</div>
<input type="hidden" name="id"   <?php echo "value='".$row['Supplier_ID']."'";?> >
